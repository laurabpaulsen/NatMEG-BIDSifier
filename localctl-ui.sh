...
show_menu() {
  ...
  echo -e "${BOLD}╔════════════════════════════════════════╗${NC}"
  if [[ "${MODE:-1}" == "1" ]]; then
    echo -e "${BOLD}║  NatMEG Tunnel Manager (Remote)${NC}"
  else
    echo -e "${BOLD}║  NatMEG Local Manager${NC}"
  fi
  echo -e "${BOLD}╚════════════════════════════════════════╝${NC}"
  echo
  ...
  if [[ "${MODE:-1}" == "1" ]]; then
    ...
    echo -e "  1) ${GREEN}Start${NC}    - Launch local application"
  fi
  echo -e "  2) ${YELLOW}Status${NC}   - Check application status"
  echo -e "  3) ${YELLOW}Stop${NC}     - Stop application"
  ...
  echo -e "  7) ${BLUE}Settings${NC} - Configure"
...
run_start() {
  ...
  if [[ "${MODE:-1}" == "1" ]]; then
    ...
    # Local mode: actually start the app
    echo -e "${BLUE}Starting local application...${NC}"
    local args=""
    [[ -n "${LOCAL_PORT:-}" ]] && args="$args --local-port $LOCAL_PORT"
    
    if "$LOCALCTL_LOCAL" start $args; then
      echo
...
while true; do
  ...
  if [[ "${MODE:-1}" == "1" ]]; then
    ...
    read -rp "Enter your choice (0-3, 7): " choice
